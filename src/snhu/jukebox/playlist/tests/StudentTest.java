...
public class StudentTest {
	...
	public void testGetStudentNameList4() {
		...
		StudentList studentList = new StudentList();									//instantiate the StudentList object so we can access it's methods and properties
		studentNames = studentList.getStudentsNames();									//populate the studentNames list with the actual values in the StudentsList object
		assertEquals("Jada Thibodeaux", studentNames.get(3));							//test case to see if the second value contains the name we expect
	}
	...
	public void testJadaThibodeauxProfile() {
		TestJadaThibodeaux_Playlist testJadaThibodeauxPlaylist = new TestJadaThibodeaux_Playlist();
		Student TestJadaThibodeaux = new Student("TestJadaThibodeaux", testJadaThibodeauxPlaylist.StudentPlaylist());
		assertEquals("JadaThibodeaux", TestJadaThibodeaux.getName());
	}
